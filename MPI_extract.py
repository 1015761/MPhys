...
failed = []
times, star_radii, tic_list, labels, n_trans = get_tics()
n_no_free = 0
...
if __name__ == '__main__':
	...
	if mpi_rank == mpi_root:
		...
		if exists('/mnt/zfsusers/blakeland/Documents/MPhys/PRF_manifest.csv'): #Check this is right
			...
			sys.stdout.flush()
			metadata_header = ['ID', 'Times', 'Keep', 'Params', 'Uncs']
	
			with open('/mnt/zfsusers/blakeland/Documents/MPhys/PRF_manifest.csv', 'w') as f: # save in the photometry folder
		...
		if (not with_mpi) or (nlc == 1) or (mpi_size ==1):
			...
			sys.stdout.flush()
			manifest_table = pd.read_csv('/mnt/zfsusers/blakeland/Documents/MPhys/PRF_manifest.csv')
			...
			for tic in times:
				for sec in times[tic]:
					...
					if not np.isin(ID, ID_exist):
						...
						sys.stdout.flush()
						res = main.get_params(f, sec, labels[f][sec])
						...
						sys.stdout.flush()
						#	continue
						with open('/mnt/zfsusers/blakeland/Documents/MPhys/PRF_manifest.csv', 'a') as file: # save in the photometry folder
							writer = csv.writer(file, delimiter=',')
				...
				while tic_list or active_workers:
					...
					for w in active_workers:
						if comm.Iprobe(w, 2):
							...
							with open('/mnt/zfsusers/blakeland/Documents/MPhys/PRF_manifest.csv', 'a') as f:
								writer = csv.writer(f, delimiter=',')
								#if res['TIC'] != -99:
								#ID_sec = "s{}".format(10000 + int(res['Sector']))[1:]
								#ID_tic = str(1e16 + int(res['TIC']))[1:]
		...
		while True:
			...
			res = main.get_params(tic_worker, sec_worker, labels[tic_worker][sec_worker])
